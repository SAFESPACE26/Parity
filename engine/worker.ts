...
async function claimRun(): Promise<{ id: string; project_id: string } | null> {
  // Atomically claim one queued run using FOR UPDATE SKIP LOCKED so multiple
  // workers (or restarts) never double-process the same run.
...
async function processRun(run: { id: string; project_id: string }): Promise<void> {
  ...
  const result = await runPipeline(run.id, run.project_id, {
    fixturesDir: DEMO_FIXTURES,
  });
  console.log(
